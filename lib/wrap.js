...
Wrap.prototype.require = function (mfile, opts) {
    ...
    if (self.has(mfile)) {
        log("Requiring " + mfile + " - already loaded");
        return self;
    }
    if (opts.target && self.has(opts.target)) {
        log("Requiring " + mfile + " - already has target");
        return self;
    }

    if (self.ignoring[mfile]) {
        log("Ignoring " + mfile);
        return self;
    }
    if (self.aliases[mfile]) {
        log("Ignoring alias " + mfile);
        return self;
    }
